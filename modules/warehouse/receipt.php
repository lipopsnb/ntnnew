...
$jobOrderItemsMap = [];
$itemSql = "SELECT joi.id, joi.job_order_id, joi.qty_received, pc.code, pc.name, pc.unit
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$errors) {
        try {
            ...
            $receiptCode = erp_generate_daily_code($pdo, 'warehouse_receipts', 'receipt_code', 'NK', $receiptDate);
            $stmt = $pdo->prepare('INSERT INTO warehouse_receipts (receipt_code, job_order_id, receipt_date, received_by, note) VALUES (?, ?, ?, ?, ?)');
            $stmt->execute([$receiptCode, $jobOrderId, $receiptDate, $receivedBy, $note ?: null]);
            $receiptId = (int) $pdo->lastInsertId();

            $insertItem = $pdo->prepare('INSERT INTO warehouse_receipt_items (receipt_id, job_order_item_id, product_code_id, qty) SELECT ?, joi.id, joi.product_code_id, ? FROM job_order_items joi WHERE joi.id = ? AND joi.job_order_id = ?');
            foreach ($items as $item) {
                $insertItem->execute([$receiptId, $item['qty'], $item['job_order_item_id'], $jobOrderId]);
            }

            $pdo->prepare("UPDATE job_orders SET status = 'in_progress' WHERE id = ? AND status = 'draft'")->execute([$jobOrderId]);
            $pdo->commit();
            erp_flash('success', 'Đã tạo phiếu nhập kho ' . $receiptCode . '.');
            erp_redirect(erp_url('modules/warehouse/receipt.php'));
...
$receipts = $pdo->query("SELECT wr.id, wr.receipt_code, wr.receipt_date, wr.received_by, jo.job_code, c.name AS customer_name
                ...
                <div class="card-body table-responsive">
                    <table class="table table-hover align-middle mb-0">
                        <thead class="table-light">
                            <tr><th>Mã phiếu</th><th>Phiếu GC</th><th>Khách hàng</th><th>Ngày nhập</th><th>Người nhận</th></tr>
                        </thead>
                        <tbody>
                        <?php if (!$receipts): ?>
                        ...
                        <?php foreach ($receipts as $receipt): ?>
                            <tr>
                                <td class="fw-semibold text-primary"><?= erp_h($receipt['receipt_code']) ?></td>
                                <td><?= erp_h($receipt['job_code']) ?></td>
                ...
                <div class="card-body">
                    <form method="post" id="receiptForm" class="row g-3">
                        ...
                        <div class="col-12">
                            ...
                            <select class="form-select" name="job_order_id" id="jobOrderSelect" required>
                        ...
                        <div class="col-12">
                            <div class="table-responsive">
                                <table class="table align-middle mb-0">
                                    <thead class="table-light"><tr><th>Mã SP</th><th>Tên SP</th><th>ĐVT</th><th>Số lượng nhập</th></tr></thead>
                                    <tbody id="receiptItemsBody">
                                        <tr><td colspan="4" class="text-center text-muted py-3">Chọn phiếu gia công để tải sản phẩm.</td></tr>
                                    </tbody>
                                </table>
                            </div>
                        </div>
...
function renderReceiptItems(jobOrderId) {
    ...
    if (!items.length) {
        body.innerHTML = '<tr><td colspan="4" class="text-center text-muted py-3">Không có chi tiết sản phẩm.</td></tr>';
        return;
    }
    body.innerHTML = items.map(item => `
        <tr>
            ...
            <td>${item.name}</td>
            <td>${item.unit}</td>
            <td><input type="number" min="0" step="0.01" class="form-control" name="qty[${item.id}]" placeholder="0"></td>
        </tr>`).join('');
}
