...
try {
    ...
    foreach ($items as $assignId => $item) {
        ...
        // ── Nếu frontend truyền lên khác (trường hợp sửa) thì ưu tiên DB
        ...
        if ($extra) {
            ...
            $reason       = $extra['reason'] ?? '';
            // Tính lại salary_actual theo salaryDay mới (tổng lương)
            if ($isDeducted && isset($item['kpi_target']) && (int)$item['kpi_target'] > 0) {
                ...
                // Đảm bảo không vượt quá salaryDay
                $salaryActual = min($salaryActual, $salaryDay);
            }
        } else {
            $isDeducted   = 0;
            $reason       = '';
